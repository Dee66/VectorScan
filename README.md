...
- **Runtime telemetry (`scan_duration_ms`) baked into every JSON payload** so CI pipelines and reliability dashboards can detect performance regressions immediately.
...
- VectorScan measures its own wall-clock runtime using a deterministic helper, publishing `metrics.scan_duration_ms` in JSON output (and `scan_duration_ms` fields in telemetry logs, summaries, CSV, and the YAML audit ledger).
- Override via `VSCAN_FORCE_DURATION_MS` when you need reproducible fixtures or deterministic CI snapshots, but by default the value reflects actual execution time (in milliseconds) for performance monitoring.
- Aggregate helpers and StatsD exports now stream the metric as gauges + timers (avg/p95/max/latest) so Datadog/Graphite dashboards can alert on slow scans or budget CPU in multi-tenant environments.
