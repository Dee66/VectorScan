...
- [x] Creates missing files automatically. _(See `tools/vectorscan/copilot_scaffolder.py` + `tests/unit/test_copilot_scaffolder_unit.py`.)_
- [x] Fills fixtures with minimal valid JSON. _(Scaffolder seeds semantic Terraform plans; validated via `tests/unit/test_copilot_scaffolder_unit.py::test_fixture_templates_embed_semantic_content`.)_
- [x] Generates golden files. _(Placeholders now include violations/IAM drift data; see `tests/unit/test_copilot_scaffolder_unit.py::test_golden_templates_embed_expected_blocks`.)_
- [x] Writes full pytest suites. _(Generated via `tools/vectorscan/copilot_suite_generator.py`; enforced by `tests/unit/test_copilot_suite_generator_unit.py` + `tests/copilot_generated/test_cli_smoke.py`.)_
- [x] Stubs missing internal APIs. _(See `tools/vectorscan/copilot_api_stubber.py`, the `tools/vectorscan/api_stubs/` package, and `tests/unit/test_copilot_api_stubber_unit.py`.)_
- [x] Enforces deterministic outputs. _(Guarded by `tools/vectorscan/copilot_determinism_guard.py` + `tests/unit/test_copilot_determinism_guard_unit.py` running the CLI twice per fixture.)_
- [x] Integrates linting/type checks. _(CI + local contributors run `scripts/run_static_checks.sh`, which chains the Copilot guards before Ruff/Black/isort/mypy.)_
- [x] Produces CI workflow YAML. _(Generated by `tools/vectorscan/copilot_workflow_generator.py`; drift caught by `tests/unit/test_copilot_workflow_generator_unit.py` + `scripts/run_static_checks.sh`.)_
