...
def _run_cosign(bundle: Path, signature: Path, public_key: Path, require: bool) -> bool:
    ...
    if not cosign:
        if require:
            ...
            return False
        print("release-verifier: cosign not found; skipping signature verification", file=sys.stderr)
        return True
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download GitHub release artifacts and verify integrity")
    parser.add_argument("--artifact-url", required=True, help="URL of the release artifact to download")
    parser.add_argument("--artifact-output", type=Path, help="Optional path to save the downloaded artifact")
    parser.add_argument("--signature-url", help="URL of the signature file (for cosign verification)")
    parser.add_argument("--signature-output", type=Path, help="Optional path to save the downloaded signature")
    ...
    parser.add_argument("--public-key", type=Path, help="Path to cosign public key for verification")
    parser.add_argument(
        "--require-cosign",
        action="store_true",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        if args.artifact_output:
            ...
            artifact_path = secure_temp_file(prefix="release-artifact-", suffix=".bin", directory=tmp_dir)
        try:
        ...
        if args.public_key or args.signature_url or args.signature_output:
            if not (args.public_key and args.signature_url):
                print("release-verifier: --public-key and --signature-url must be provided together", file=sys.stderr)
                return EXIT_INVALID_INPUT
            if args.signature_output:
                signature_path = args.signature_output
                signature_path.parent.mkdir(parents=True, exist_ok=True)
            else:
                signature_path = secure_temp_file(prefix="release-signature-", suffix=".sig", directory=tmp_dir)
            try:
