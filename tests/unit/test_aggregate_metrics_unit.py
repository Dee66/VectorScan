...
@given(
    obj=st.one_of(
        st.dictionaries(st.text(), st.integers() | st.text() | st.none() | st.lists(st.integers() | st.text() | st.none()), max_size=5),
...
def test_main_stress_mixed(tmp_path):
    ...
    for i in range(50):
        ...
        f.write_text(json.dumps({"status": status, "violations": [] if status == "PASS" else [f"P-SEC-{i:03d}"]}), encoding="utf-8")
        files.append(f)
    orig_glob = glob.glob
...
@given(st.dictionaries(st.text(), st.integers() | st.text() | st.none() | st.lists(st.integers() | st.text() | st.none()), max_size=5))
...
import os
import pytest
from aggregate_metrics import load_json, extract_result, main
...
@pytest.mark.parametrize("content,expected", [
    ...
    (json.dumps({"status": "FAIL", "violations": ["P-SEC-001"]}), {"status": "FAIL", "violations": ["P-SEC-001"]}),
...
@pytest.mark.parametrize("obj,expected", [
    ...
    ({"status": "FAIL", "violations": ["P-SEC-001: ..."]}, {"status": "FAIL", "violations": ["P-SEC-001: ..."]}),
    ({}, None),
    ([], None),
    (None, None),
    ({"result": None}, None),
    ({"status": "PASS"}, None),
    ({"violations": []}, None),
    ({"result": {"status": "FAIL", "violations": ["P-SEC-002"]}}, {"status": "FAIL", "violations": ["P-SEC-002"]}),
...
def test_main_summary(tmp_path):
    ...
    f2.write_text(json.dumps({"status": "FAIL", "violations": ["P-SEC-001: ..."]}), encoding="utf-8")
    import glob
    orig_glob = glob.glob
...
def test_main_out_file(tmp_path):
    ...
    code = main(["--glob", str(f1), "--out", str(out_file)])
    builtins.print = orig_print
    glob.glob = orig_glob
